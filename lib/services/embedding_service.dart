...
import '../services/tflite_ffi_handler.dart';

enum EmbeddingBackend { tflite }

class EmbeddingService {
  final LogService _logService;
  final TfliteFfiHandler _ffiHandler;
  ...
  Future<bool> initialize(
    ...
    _logService.log('EmbeddingService', '=== Initializing embedding service (FFI) ===');
    ...
    try {
      ...
      _logService.log('EmbeddingService', 'Initializing FFI handler...');
      final success = await _ffiHandler.initialize(modelPath, vocabPath: vocabPath);

      if (success) {
        _activeBackend = EmbeddingBackend.tflite;
        _currentModelPath = modelPath;
        _vocabPath = vocabPath;
        _embeddingDimension = _ffiHandler.embeddingDimension;
        _isInitialized = true;
  ...
  Future<List<double>> embed(String text) async {
    ...
    try {
      final result = await _ffiHandler.embed(text);
      if (result == null) {
        _logService.log('EmbeddingService', 'ERROR: Embedding returned null');
        throw Exception('Embedding returned null');
      }
      _logService.log('EmbeddingService', 'Embedding OK: dim=${result.length}');
  ...
  Future<List<List<double>>> embedBatch(List<String> texts) async {
    ...
    try {
      final results = await _ffiHandler.embedBatch(texts);
      final validResults = results
          .where((r) => r != null)
          .map((r) => r!.toList())
          .toList();
      _logService.log('EmbeddingService', 'Batch complete: ${validResults.length}/${texts.length} successful');
      return validResults;
    } catch (e, stackTrace) {
  ...
  void dispose() {
    _logService.log('EmbeddingService', 'Disposing...');
    _ffiHandler.close();
